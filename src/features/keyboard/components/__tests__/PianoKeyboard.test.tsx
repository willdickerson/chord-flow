...
describe('PianoKeyboard', () => {
  ...
  it('renders white keys with correct styling', () => {
    ...
    const svg = document.querySelector('svg')
    const whiteKeys = Array.from(svg?.querySelectorAll('rect') || []).filter(
      key => key.getAttribute('fill') === 'white'
    )
  ...
  it('highlights active notes with purple color', () => {
    ...
    const svg = document.querySelector('svg')
    const purpleKeys = Array.from(svg?.querySelectorAll('rect') || []).filter(
      key => key.getAttribute('fill') === '#9333ea'
    )
  ...
  it('renders SVG with correct dimensions and properties', () => {
    ...
    expect(svg).toHaveAttribute(
      'height',
      KEYBOARD_CONFIG.whiteKeyHeight.toString()
    )
  })
  ...
  it('positions black keys relative to white keys', () => {
    ...
    const svg = document.querySelector('svg')
    const blackKeys = Array.from(svg?.querySelectorAll('rect') || []).filter(
      key =>
        !key.getAttribute('fill')?.includes('white') &&
